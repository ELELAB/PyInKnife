...
new.df <- t(unique(new.df[rowSums(is.na(new.df)) != ncol(new.df),1:13]))
...
new.df<- new.df[-1,]
row.names(new.df) <- c("Trajectory", "Resampling_0", "Resampling_1","Resampling_2","Resampling_3","Resampling_4","Resampling_5",
                        "Resampling_6","Resampling_7","Resampling_8","Resampling_9")

###### TRANSFORM

for (n in seq(1, dim(new.df)[1],1)){
  #print(n)
  data_numn <- factor(as.numeric(new.df[n,]))
  tbn <- table(data_numn)
  sorted_n <- factor(data_numn,levels = names(tbn[order(tbn, decreasing = TRUE)]))
  levels(sorted_n)[1:5] <- as.numeric(seq(1,5))
  new.df[n,] <- sorted_n
}
