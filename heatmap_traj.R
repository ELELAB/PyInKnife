...
#    PyinKnife script to plot heatmap for the jack-knife resampling
...
my_palette <- colorRampPalette(c("royalblue4", "yellow", "orange" ,"red2"))(n = 150)
pdf(args[2])
...
print(levelplot(t(new.df[,as.numeric(args[3]):as.numeric(args[4])]),
          ...
          main = " ",
          #key = my.ckey,
          at = seq(0,max(as.numeric(levels(factor(new.df))))),
